...
const SearchScreen = () => {
  ...
  //This function returns 12 Meals
  // Checks weather it was searched by name or by ingredient or Random Meals if no search
  const performSearch = async (query: string) => {
    //No search user input, fetch random meals
    if (!query.trim()) {
  ...
  useEffect(() => {
    ...
    const handleSearch = async () => {
      try {
        setLoading(true);
        const results = await performSearch(debouncedValue);
        setRecipes(results);
      } catch (error) {
